...
or $\{V \circ V:V \in \mathcal{U}\}$ forms a base of $\mathcal{U}$.
...
In fact, for any $x,y \in X$ and any $U \in \mathcal{U}$, we have
$$ U(y) = U \circ \{y\} = U \circ \Delta_X (y) \supset U \circ U (x). $$
By the condition (c) in Definition~\eqref{def2.1}, there exists $V \in \mathcal{U}$ such that $V \circ V \subset U$.
Thus we have $V(x) \subset U(y)$. By symmetry, we also have $V(y) \subset U(x)$.
