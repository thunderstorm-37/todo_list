...
We can also prove that~$\nabla^2 f(x) \succeq 0$ implies the strictly convexity of~$f$. However, the converse is not true. For example, the function~$f(x) = x^4$ is a strictly convex function but its second-order derivative is not positive semidefinite at~$x=0$.
